            ...
            if api_settings.PASSWORDLESS_REGISTER_NEW_USERS is True:
                ...
                try:
                    user = User.objects.get(
                        **{self.alias_type + "__iexact": alias}
                    )
                ...
                # افیلیت: ثبت دعوت در صورت کاربر جدید و کد معرف معتبر (بدون حساسیت به حروف بزرگ/کوچک)
                if created and request:
                    referral_code = (request.data.get("referral_code") or request.query_params.get("ref") or "").strip().upper()
                    if referral_code:
                        try:
                            referrer = User.objects.get(referral_code__iexact=referral_code)
                            if referrer.pk != user.pk:
                                from affiliate.models import AffiliateInvite
                                AffiliateInvite.objects.get_or_create(
                                    inviter=referrer, invitee=user,
                                    defaults={"inviter": referrer, "invitee": user}
                                )
...
class AbstractBaseCallbackTokenSerializer(serializers.Serializer):
    ...
    token = TokenField(min_length=5, max_length=5, validators=[token_age_validator])
