...
def has_active_token(user, token_type):
    """
    Returns True if user has an active (non-expired) token of the given type.
    If an active token exists, silently ignore new requests (return True without sending).
    """
    try:
        # Get the most recent active token for this user and type
        # Since signals invalidate old tokens when new ones are created,
        # there should typically be at most one active token
        active_token = CallbackToken.objects.active().filter(
            user=user,
            type=token_type
        ...
        seconds = (timezone.now() - active_token.created_at).total_seconds()
        ...
        if seconds <= token_expiry_time:
            # Token is still active and valid
            return True
        else:
...
def create_authentication_jwt_token(user):
    """Create JWT token instead of default token"""
    refresh = RefreshToken.for_user(user)
    return {
        "refresh": str(refresh),
        "access": str(refresh.access_token),
    }, True  # created=True چون همیشه یک توکن جدید ایجاد می‌کنیم
