...
function mediaFromArticle(article: Article, type: "main" | "thumbnail"): Media | null {
  const m = type === "main" ? article.main_image : article.thumbnail_image;
  if (!m?.id || !m?.file) return null;
  return {
    id: m.id,
    title: "",
    description: "",
    file: m.file.startsWith("http") ? m.file : `${API_BASE.replace(/\/$/, "")}${m.file.startsWith("/") ? "" : "/"}${m.file}`,
    file_type: "image/*",
    file_size: 0,
    hash_sum: "",
    uploaded_at: "",
...
export default function EditArticlePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([a, catRes, tagsList]) => {
        setArticle(a);
        setForm({
          title: a.title,
          slug: a.slug,
          description: a.description ?? "",
          status: (a.status as "draft" | "public") ?? "draft",
          category: a.category?.id ?? "",
          meta_title: a.meta_title ?? "",
          meta_description: a.meta_description ?? "",
        });
        setMainImage(mediaFromArticle(a, "main"));
        setThumbnailImage(mediaFromArticle(a, "thumbnail"));
        setTags((a.tags ?? []).map((t: { id: string } | string) => (typeof t === "string" ? t : t.id)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await articleApi.update(article.slug, {
        title: form.title,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold">{tFrontendAuto("fe.0ff4e85d0645")}</h1>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="card p-6 space-y-4">
              ...
              <p className="text-xs text-gray-500">
                بازدید: {(article.total_views ?? 0).toLocaleString()}
              </p>
            </div>
