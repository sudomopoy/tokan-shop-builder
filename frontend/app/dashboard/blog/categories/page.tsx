...
export default function BlogCategoriesPage() {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await categoryApi.list({ module: "BLOG", page_size: 200 });
      const cats = res.results ?? [];
      const treeRes = await categoryApi.tree({ module: "BLOG" }).catch(() => cats);
  ...
  function flattenCategories(
    ...
    for (const c of items) {
      if (c.module !== "BLOG" && c.module !== "blog") continue;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modal === "create") {
        await categoryApi.create({ name: form.name, module: "BLOG" });
      } else if (editCategory) {
        await categoryApi.update(editCategory.id, { name: form.name });
      }
      closeModal();
  ...
  const handleDelete = async (cat: Category) => {
    ...
    try {
      ...
      console.error(err);
      alert("خطا در حذف دسته. ممکن است مقالاتی به آن اختصاص داشته باشند.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-gray-600 text-sm">
        دسته‌بندی‌ها به شما کمک می‌کنند مقالات بلاگ را سازماندهی کنید.
      </p>
